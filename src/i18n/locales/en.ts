// PATTERN: English translations
export const en = {
  // Navigation
  "myIdeas": "My Ideas",
  "addIdea": "Add Idea",
  "settings": "Settings",
  
  // Ideas Screen
  "noIdeasYet": "No ideas yet",
  "tapToAddFirstIdea": "Tap + to add your first idea",
  "loadingMoreIdeas": "Loading more ideas...",
  
  // Add Idea Screen
  "addNewIdea": "Add New Idea",
  "ideaTitle": "Idea Title",
  "ideaTitlePlaceholder": "Ex: Recipe mobile app",
  "ideaDescription": "Description",
  "ideaDescriptionPlaceholder": "Ex: An app to discover and share recipes with rating system and automatic shopping list",
  "saveIdea": "Save Idea",
  "cancel": "Cancel",
  
  // Settings Screen
  "language": "Language",
  "french": "Français",
  "english": "English",
  "about": "About",
  "version": "Version",
  
  // Error Messages
  "saveError": "Failed to save idea. Please try again.",
  "loadError": "Failed to load ideas. Please try again.",
  "validationError": "Please fill in all required fields.",
  "titleTooLong": "Title must be less than {{max}} characters",
  "descriptionTooLong": "Description must be less than {{max}} characters",
  "retry": "Retry",
  
  // Actions
  "add": "Add",
  "edit": "Edit",
  "delete": "Delete",
  "save": "Save",
  "close": "Close",
  
  // Floating Action Button
  "addNewIdeaFab": "Add new idea",
  
  // Rating
  "rating": "Rating",
  "optional": "optional",
  "clearRating": "Clear rating",
  
  // Filters
  "allRatings": "All ratings",
  "newestFirst": "Newest first",
  "oldestFirst": "Oldest first",
  "highestRated": "Highest rated",
  "lowestRated": "Lowest rated",
  "clearFilters": "Clear",
};